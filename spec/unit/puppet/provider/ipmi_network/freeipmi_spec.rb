...
describe Puppet::Type.type(:ipmi_network).provider(:freeipmi) do
  ...
  describe 'property setters' do
    ...
    it 'sets type to dhcp' do
      provider.expects(:bmcconfig_exec)
              .with(%w[--commit --key-pair Lan_Conf:IP_Address_Source=Use_DHCP --lan-channel-number 1], failonfail: true)
    ...
    it 'sets type to static' do
      provider.expects(:bmcconfig_exec)
              .with(%w[--commit --key-pair Lan_Conf:IP_Address_Source=Static --lan-channel-number 1], failonfail: true)
    ...
    it 'sets the ip address' do
      provider.expects(:bmcconfig_exec)
              .with(%w[--commit --key-pair Lan_Conf:IP_Address=192.168.1.100 --lan-channel-number 1], failonfail: true)
    ...
    it 'sets the netmask' do
      provider.expects(:bmcconfig_exec)
              .with(%w[--commit --key-pair Lan_Conf:Subnet_Mask=255.255.255.0 --lan-channel-number 1], failonfail: true)
    ...
    it 'sets the gateway' do
      provider.expects(:bmcconfig_exec)
              .with(%w[--commit --key-pair Lan_Conf:Default_Gateway_IP_Address=192.168.1.1 --lan-channel-number 1], failonfail: true)
